...
void sendDataToFirebase(float temperature, float humidity, float CO2, float light) 
{
  if (client.connect(firebaseHost,443)) { // Use client.connect() with host and port
    ...
    String dataToSend = "{\"temperature\": " + String(temperature, 2) + ", \"humidity\": " + String(humidity, 2) + ", \"CO2\": " + String(CO2, 2)  + ", \"light\": " + String(light, 2)+"}";
...
void loop() {
  ...
  sendDataToFirebase(temperature, humidity,CO2,light);
